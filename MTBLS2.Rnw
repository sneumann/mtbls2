...
\author{Kenneth Haug \and Reza M. Salek \and Pablo Conesa \and
...
Since 2006, the Bioconductor package \Rpackage{xcms} \citep{Smith06XCMSProcessingmass} provides a rich set of
...
which was described in \cite{Neumann12Nearlineacquisitionand}.
...
With the combination of \Rpackage{Risa} and \Rpackage{xcms}, we can convert the MS raw data 
in an ISAtab archive into an \Rclass{xcmsSet}:

<<PeakPicking, results = hide, cache=true, cache=true>>=
...
<<RTcheckandGroup, results=hide, cache=true>>=
...
in the mzTab format used in MetaboLights.

Alternatively, the xcmsSet can be written out as PSI mzQuantML format:
<<mzQuantML, results=hide, cache=true>>=
source("writemzq.R")
system(command="rm -f MTBLS2.mzq.xml")
write.mzq(mtbls2Set, "MTBLS2.mzq.xml")
@


In a typical mass spectrometry metabolomics workflow, the features
detected by \Rpackage{xcms} are further processed by \Rpackage{CAMERA} \citep{Kuhl12CAMERAintegratedstrategy}, another Bioconductor
package. \Rpackage{CAMERA} provides several algorithms to determine which
...
<<CAMERA annotation and Stats, results=hide, cache=true>>=
...
``modification'' columns. Here, we use 

...
specification\footnote{\url{http://www.psidev.info/index.php?q=node/80}}. Here, we use 
the m/z and retention of the xcmsSet groups. As intensities, we can use any of the 
alternative quantification approaches from groupval(). From CAMERA, we obtain the charge, 
deduced from the isotopic pattern. Many of the other mzTab columns are left empty.

Finally, the constructed dataframe is written to a CSV file. If for some of the masses additional identification information was obtained, e.g. through MS/MS measurements, this information can be added after

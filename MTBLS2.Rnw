...
\author{Kenneth Haug \and Reza M. Salek \and Pablo Conesa \and
  ...
  Tejasvi Mahendraker \and Mark Williams \and Steffen Neumann \and
...
<<LibraryPreload, echo = FALSE, results = hide, cache=true>>=
library(Risa)
library(xcms)
library(CAMERA)
@
...
<<RTcheckandGroup, fig = TRUE, eps = FALSE, cache=true>>=
mtbls2Set <- group(mtbls2Set, minfrac=0.75, bw=2)
retcor(mtbls2Set, plottype="mdevden")
@

<<fillPeak, results = hide, cache=true>>=
